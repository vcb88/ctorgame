...
import { NeonBillboard } from '@/components/display/NeonBillboard.js';
import { NeonGridBackground } from '@/components/backgrounds/NeonGridBackground.js';
...
type JoinGameErrorType = GameError & {
    retryable?: boolean;
};
...
export const WaitingRoom: React.FC = () => {
    ...
    const joinWithRetry = async (gameId: string, retryAttempt: number = 0) => {
        try {
            ...
            if (retryAttempt < MAX_JOIN_RETRIES && error.retryable !== false) {
                ...
                setTimeout(() => {
                    joinWithRetry(gameId, retryAttempt + 1);
                }, RETRY_DELAY);
            } else {
                setIsJoining(false);
    ...
    const formatPlayerNumber = (num: PlayerNumber | null): string => 
        num ? String(num) : '...';
                    ...
                    <div className="flex justify-between items-center text-xs font-mono">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="text-white/50">
                                {playerNumber !== null 
                                    ? `ID: ${formatPlayerNumber(playerNumber)}` 
                                    : 'INITIALIZING'}
                            </div>
                        </div>
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-cyan-400 text-sm tracking-wider">
                                        {playerNumber ? `PLAYER ${formatPlayerNumber(playerNumber)}` : 'CONNECTING...'}
                                    </span>
                                </div>
